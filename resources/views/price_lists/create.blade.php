        ...
        <div class="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100 relative overflow-hidden">
            ...
            <div class="space-y-4 relative z-10">
                ...
                <div class="grid grid-cols-1 gap-4">
                    ...
                    @php 
                        // جلب التكلفة من جدول product_costs
                    ...
                    <div class="group flex flex-col md:flex-row items-start md:items-center justify-between p-6 bg-slate-50 rounded-[2rem] border border-slate-100 hover:border-indigo-200 hover:bg-white transition-all gap-4">
                        <div class="flex items-center gap-4">
                            <div class="w-10 h-10 bg-white rounded-xl shadow-sm flex items-center justify-center text-slate-300 group-hover:text-indigo-500 transition-colors">
                                <i class="fa-solid fa-box"></i>
                            </div>
                            <div class="flex flex-col">
                                <span class="font-black text-slate-700 group-hover:text-indigo-600 transition-colors tracking-tight">{{ $product->name }}</span>
                                <span class="text-[10px] text-amber-600 font-black uppercase flex items-center gap-1 mt-1 bg-amber-50 px-2 py-0.5 rounded-md border border-amber-100 w-fit">
                                    <i class="fa-solid fa-hand-holding-dollar text-[8px]"></i> 
                                    متوسط التكلفة: {{ number_format($cost) }} SDG
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="relative w-full md:w-auto">
                            ...
                            <input type="number" step="0.01" name="prices[{{ $product->id }}][price]" 
                                   class="w-full md:w-40 p-4 pr-10 bg-white border-2 border-slate-200 rounded-2xl text-center font-black text-indigo-600 outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 shadow-sm transition-all" 
                                   placeholder="0.00">
                            <span class="absolute top-1/2 -translate-y-1/2 left-4 text-[9px] font-black text-slate-400 uppercase">SDG</span>
                        </div>
                    </div>
                    @endforeach
                </div>
            ...
            <div class="mt-12 flex gap-4">
                <button type="submit" class="flex-[2] bg-indigo-600 text-white py-5 rounded-[1.5rem] font-black shadow-xl shadow-indigo-100 hover:bg-indigo-700 hover:-translate-y-1 transition-all flex items-center justify-center gap-3">
                    <i class="fa-solid fa-check-circle text-lg"></i>
                    حفظ واعتماد القائمة
                </button>
                <a href="{{ route('price-lists.index') }}" class="flex-1 bg-slate-100 text-slate-500 py-5 rounded-[1.5rem] font-black hover:bg-slate-200 transition-all text-center flex items-center justify-center">
                    إلغاء
                </a>
            </div>
        </div>
